...
function PendingFriendComp({ fri, updateHandler }) {
  ...
  const acceptHandle = () => {
    userService.acceptFriendRequest(user.username, fri._id).then((resp) => {
      updateHandler(Math.random() * 100000);
    });
  };

  const declinetHandle = () => {
    userService.declineFriendRequest(user.username, fri._id).then((resp) => {
      updateHandler(Math.random() * 100000);
    });
  };
  ...
  return (
    <div className="pendingFriendCard">
      <Link to={"/" + fri.username + "/profile"} className="">
        <img
          className="profilePicPending"
          src={fri.profileImage}
          alt={fri.name}
        />
      </Link>
      ...
      <div className="RequestButtons">
        <button
          className="btn btn-primary rqButtPending"
          onClick={acceptHandle}
        >
          Accept Request
        </button>
        <button
          className="btn btn-secondary rqButtPending"
